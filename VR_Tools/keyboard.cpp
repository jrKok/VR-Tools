...
*/
    ...
    string fileName("");
    if (numpad){
        fileName="Resources/plugins/VR_Tools/ressources/keyboards/numpad2.cfg";}
    else{
        fileName="Resources/plugins/VR_Tools/ressources/keyboards/USKB.cfg";
...
int  Keyboard::Physical_Key_Handler(char in_char,XPLMKeyFlags flag,char in_VKs,void* inRefcon){
     ...
     * processes one discreet keypress (simultaneous keypresses will only be handled for first key)
     ...
     * */
     ...
     if ((flag&XPLM_KEY_DOWN)&&!(in_VK==activeVKCode)){//a different key is pressed
